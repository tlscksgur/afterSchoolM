    ...
    <div class="t-modal-overlay" id="courseManagementModal">
      <div class="t-modal-content large">
        <div class="t-modal-header">
          ...
          <button class="t-modal-close" onclick="closeCourseManagementModal()">&times;</button>
        </div>
...
function renderAttendanceGrid(students, attendanceData) {
  ...
  container.innerHTML = `
    <table class="table">
      ...
      <tbody>
        ${students.map(s => {
    const attendance = attendanceData.find(a => a.studentId === s.studentId || a.enrollmentId === s.enrollmentId);
    const status = attendance ? attendance.status : 'NONE';

    return `
            <tr>
              <td>${s.studentName || s.name}</td>
              <td><input type="radio" name="attendance_${s.enrollmentId || s.studentId}" value="PRESENT" ${status === 'PRESENT' ? 'checked' : ''} /></td>
              <td><input type="radio" name="attendance_${s.enrollmentId || s.studentId}" value="ABSENT" ${status === 'ABSENT' ? 'checked' : ''} /></td>
              <td><input type="radio" name="attendance_${s.enrollmentId || s.studentId}" value="LATE" ${status === 'LATE' ? 'checked' : ''} /></td>
            </tr>
          `;
  }).join('')}
      </tbody>
...
async function saveAttendanceData() {
  ...
  students.forEach(s => {
    const enrollmentId = s.enrollmentId || s.studentId;
    const selected = document.querySelector(`input[name="attendance_${enrollmentId}"]:checked`);
    if (selected) {
  ...
  const modalHTML = `
    <div class="t-modal-overlay" id="courseEditModal">
      <div class="t-modal-content">
        <div class="t-modal-header">
          <h2>강좌 수정</h2>
          <button class="t-modal-close" onclick="closeCourseEditModal()">&times;</button>
        </div>
        <div class="t-modal-body">
