...
const API_BASE_URL = (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1')
  ...
  : 'https://listings-editing-postings-characterization.trycloudflare.com'; // Production backend
...
async function apiRequest(endpoint, options = {}) {
  ...
  // 인증 토큰이 있으면 헤더에 추가 (auth 관련 엔드포인트 제외)
  ...
  try {
    ...
    if (contentType && contentType.includes('application/json')) {
      ...
      if (!response.ok) {
        // Spring @Valid 에러 응답 형식에 맞춰 에러 메시지 추출
        if (data.errors && Array.isArray(data.errors) && data.errors.length > 0) {
          // 첫 번째 에러 메시지를 사용
          throw new Error(data.errors[0].defaultMessage || '입력값을 확인해주세요.');
        }
